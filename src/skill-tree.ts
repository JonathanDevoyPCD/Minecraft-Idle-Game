...
export interface SkillMilestoneRule {
  trigger: 'settlement-hub';
  required: number;
}
...
function branchEntry(branch: SkillBranchId, title: string, requiredHubLevel: number): SkillNodeDefinition {
...
const harvesting: SkillNodeDefinition[] = [
  branchEntry('harvesting', 'Harvesting', 1),
...
const toolsCrafting: SkillNodeDefinition[] = [
  branchEntry('tools-crafting', 'Tools and Crafting', 1),
...
const materialsDeepMining: SkillNodeDefinition[] = [
  branchEntry('materials-deep-mining', 'Materials and Deep Mining', 4),
...
const automation: SkillNodeDefinition[] = [
  branchEntry('automation', 'Automation', 3),
  ...
  node('automation', 'automation-mine-carts', 'Mine Cart Handling I–III', 'Improve the single minecart route without adding duplicate carts.', '+1 cargo roll per rank.', 'rank', [SKILL_TREE_BRANCH_ENTRY_IDS.automation, 'world-cave-entrance'], 3, 'Improves the existing one-cart route by increasing its cargo capacity.'),
...
const worldGrowth: SkillNodeDefinition[] = [
  branchEntry('world-growth-biomes', 'World Growth and Biomes', 1),
  ...
  node('world-growth-biomes', 'world-second-mine-site', 'Multi-Mine Coordination', 'Learn to coordinate the additional mine permits granted by later Settlement Hub eras.', 'Improves multi-mine logistics.', 'unlock', ['world-cave-entrance']),
  node('world-growth-biomes', 'world-third-mine-site', 'Mining District Planning', 'Plan a coordinated mining district as the settlement grows into a city.', 'Improves mining district logistics.', 'unlock', ['world-second-mine-site']),
...
// The first surface build consumes the World Power generated by opening the
// initial adjacent cell. This keeps CP for node choice, resources for recipes,
// and World Power for major terrain commitments.
worldGrowth.find((entry) => entry.id === 'world-surface-3x3')!.cost.worldPower = 1;

const lifeSettlement: SkillNodeDefinition[] = [
  branchEntry('life-settlement', 'Life and Settlement', 2),
...
const masteryLongTerm: SkillNodeDefinition[] = [
  branchEntry('mastery-long-term', 'Mastery and Long-Term Progression', 5),
